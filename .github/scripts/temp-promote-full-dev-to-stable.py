...
public final class NotificationSyncJobService extends JobService {
    ...
    @Override public boolean onStartJob(final JobParameters params) {
        AppExecutors.network().execute(new Runnable() {
            @Override public void run() {
                try { syncNow(); }
                catch (Throwable t) { AppLogger.warn(NotificationSyncJobService.this, "background_notification_sync", "job-failed | " + t.getClass().getSimpleName()); }
    ...
    private void syncNow() throws Exception {
        ...
        if (userId == null || userId.trim().isEmpty()) { AppLogger.info(this, "background_notification_sync", "no-session"); return; }
...
if count != 1: raise SystemExit('ForumNotificationSync delivery count declaration not found')
...
if old not in s: raise SystemExit('MainActivity showChecking recovery block not found')
s = s.replace(old, new, 1)
old = '''        String url;\n        if (i == this.connectionUiGeneration && this.startupProgress.getVisibility() == 0) {'''
new = '''        String url = null;\n        if (i == this.connectionUiGeneration && this.startupProgress.getVisibility() == 0) {'''
if old not in s: raise SystemExit('MainActivity timeout URL declaration not found')
s = s.replace(old, new, 1)
...
if old not in s: raise SystemExit('MainActivity avatar connection declaration not found')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')
...
if old not in s: raise SystemExit('LiveForumUpdater fingerprint recovery block not found')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')
...
if old not in s: raise SystemExit('ForumNotificationClient readableValue JSON block not found')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')
...
if old not in s: raise SystemExit('HcfIntentChooser resolveLabel recovery block not found')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')
...
if old not in s: raise SystemExit('MediaViewerActivity safeHttps recovery block not found')
s = s.replace(old, new, 1)
p.write_text(s, encoding='utf-8')
