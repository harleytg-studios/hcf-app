...
path = Path("source code/src/com/harleytg/forum/HcfSubActivities.java")
source = path.read_text(encoding="utf-8")

old_search = '''                new SettingTarget("permission_status", "Permission status", "permission security status", "advanced", "permissions_security"),
...
new_search = '''                new SettingTarget("permission_status", "Android permission status", "permission security status foreground service boot network", "advanced", "permissions_security"),
...
if source.count(old_search) != 1:
    raise SystemExit(f"permission search block count={source.count(old_search)}")
source = source.replace(old_search, new_search, 1)

old_card = '''        private View securityCard() {
...
new_card = '''        private View securityCard() {
            LinearLayout card = card();
...
if source.count(old_card) != 1:
    raise SystemExit(f"securityCard block count={source.count(old_card)}")
source = source.replace(old_card, new_card, 1)

old_refresh = '            if (securityStatus != null) securityStatus.setText(AppSecurity.securitySummary(this));'
new_refresh = '            if (securityStatus != null) securityStatus.setText(permissionSecuritySummary());'
if source.count(old_refresh) != 1:
    raise SystemExit(f"security refresh count={source.count(old_refresh)}")
source = source.replace(old_refresh, new_refresh, 1)

path.write_text(source, encoding="utf-8")
print("Permissions & Security patch applied")
