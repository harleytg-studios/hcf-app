 ...
 * used throughout HCF App Settings while keeping the existing forum navigation
...
public final class HcfAuthenticatorSettings {
    ...
    private static void inject(Activity activity) {
        ...
        LinearLayout profileBody = body(activity);
        profileBody.addView(detail(activity,
                "Open your public Harley's Clan Forum profile, activity and account identity."));
        styleExistingAction(activity, profileAction);
        profileBody.addView(profileAction, lp(activity, -1, dp(activity, 52), 10, 0));
        card.addView(subsetting(activity,
                "profile",
                "Forum Profile",
                "Signed in as " + handle,
                profileBody,
                true), lp(activity, -1, -2, 0, 9));

        LinearLayout securityBody = body(activity);
        securityBody.addView(securityStatusRow(activity, "Password",
                security.passwordControls ? "Available" : forumState(security),
                security.passwordControls));
        securityBody.addView(securityStatusRow(activity, "Email",
                security.emailControls ? "Available" : forumState(security),
                security.emailControls), lp(activity, -1, -2, 7, 0));
        securityBody.addView(securityStatusRow(activity, "Active sessions",
        ...
        if (!security.seen) {
            securityBody.addView(detail(activity,
        ...
        securityBody.addView(securityAction, lp(activity, -1, dp(activity, 52), 11, 0));
        String securitySummary = security.seen
        ...
        auth.setOnClickListener(v -> activity.startActivity(
                new Intent(activity, HcfAuthenticator.Activity.class)));
        twoFactorBody.addView(auth, lp(activity, -1, dp(activity, 48), 11, 0));
        ...
        twoFactorBody.addView(offline, lp(activity, -1, -2, 8, 0));

        String twoFactorSummary = isConfigured(activity)
                ? "Configured on this device"
                : (security.twoFactorControls
                        ? "Available • set up HCF Authenticator"
                        : "Set up HCF Authenticator");
        View twoFactorPanel = subsetting(activity,
                "two_factor",
                "Two-Factor Authentication",
                twoFactorSummary,
                twoFactorBody,
                true);
        TextView summary = findText(twoFactorPanel, twoFactorSummary);
        if (summary != null) summary.setTag(TAG_2FA_SUMMARY);
        card.addView(twoFactorPanel, lp(activity, -1, -2, 0, 0));
    }
    ...
    private static View subsetting(Activity activity, String key, String title,
        ...
        TextView sub = text(activity, summary, 10, color(activity, R.color.hcf_muted, Color.LTGRAY));
        sub.setMaxLines(2);
        labels.addView(sub, lp(activity, -1, -2, 3, 0));
