...
public class MainActivity extends ThemedActivity {
    ...
    private void bindViews() {
        ...
        if (drawerVersionText != null) drawerVersionText.setText("Harley's Clan Forum • " + BuildInfo.VERSION_TAG);
    ...
    private void showStableUpdateAvailableDialog(UpdateChecker.Release release) {
        ...
        String body = "A newer Harley's Clan Forum stable build is ready.\n\n"
                + "Installed: v" + BuildInfo.VERSION + " (" + BuildInfo.VERSION_CODE + ")\n"
                + "Available: v" + UpdateChecker.displayVersion(release) + " (" + availableCode + ")\n\n"
                + "Channel: Stable\n"
                + "Update when you're ready to install the latest stable build.";
        new AlertDialog.Builder(this)
                .setTitle("Stable Update Available")
                .setMessage(body)
    ...
    private void startNativeUpdateFlow() {
        ...
        message.setText("Checking Stable updates…");
        ...
        UpdateChecker.check(this, UpdateChecker.CHANNEL_STABLE, new UpdateChecker.Callback() {
            @Override
            public void onResult(UpdateChecker.Release release, boolean updateAvailable) {
                ...
                if (!updateAvailable) {
                    if (UpdateChecker.compareReleaseToInstalled(release) < 0) {
                        message.setText("Installed build " + BuildInfo.VERSION + " (" + BuildInfo.VERSION_CODE + ") is newer than the Stable feed" + (release.versionCode > 0 ? " (" + release.versionCode + ")" : "") + ". No downgrade will be installed.");
                    } else {
                        message.setText("You're on the newest Stable build.\n\nInstalled: v" + BuildInfo.VERSION + " (" + BuildInfo.VERSION_CODE + ")");
                    }
                    nativeUpdateFlowActive = false;
                    return;
                }
                if (release.apkUrl == null || release.apkUrl.trim().isEmpty()) {
                    message.setText("A newer Stable build is published, but the release does not contain an installable APK.");
                    nativeUpdateFlowActive = false;
                    return;
                }
