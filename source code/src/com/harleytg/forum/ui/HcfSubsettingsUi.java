...
/**
 * HCF_SUBSETTINGS_UI_V3_NEARATA_TWOFACTOR
 *
 * Dedicated native owner for App Settings > Account & Security > Account Controls.
 * Authentication/TOTP behavior stays in HcfAuthenticator. This class owns only
 * the Account Controls presentation, subsetting state and forum handoff UI.
 *
 * The forum uses nearata/flarum-ext-twofactor. Do not treat a locally saved TOTP
 * secret as proof that Nearata 2FA has been enabled server-side: Nearata requires
 * the user's forum password, the generated passcode and the setup secret before
 * activation, and can then issue backup codes.
 */
public final class HcfSubsettingsUi {
    private static final String TAG = "hcf_account_controls_subsettings_ui_v3_nearata";
    ...
    private static View buildTwoFactorSubsetting(Activity activity, ForumSecurity.Snapshot security) {
        LinearLayout body = body(activity);
        boolean localReady = isConfigured(activity);

        TextView provider = text(activity, "FORUM 2FA • NEARATA TWOFACTOR", 9,
                color(activity, R.color.hcf_cyan_bright, Color.rgb(0, 184, 240)));
        provider.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        body.addView(provider);

        body.addView(securityStatusRow(activity,
                "Nearata 2FA controls",
                security.twoFactorControls ? "Detected" : "Check forum",
                security.twoFactorControls),
                lp(activity, -1, -2, 7, 0));

        TextView authLabel = text(activity, "HCF AUTHENTICATOR • LOCAL DEVICE", 9,
                color(activity, R.color.hcf_cyan_bright, Color.rgb(0, 184, 240)));
        authLabel.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        body.addView(authLabel, lp(activity, -1, -2, 10, 0));

        TextView status = twoFactorStatus(activity);
        status.setTag(TAG_2FA_STATUS);
        body.addView(status, lp(activity, -1, -2, 7, 0));

        body.addView(infoBox(activity,
                "Nearata TwoFactor setup",
                "Nearata generates a TOTP QR code and manual Setup key for your forum account. HCF Authenticator can scan/import that QR code or save the Setup key and generate the required 6-digit passcode."),
                lp(activity, -1, -2, 10, 0));

        body.addView(stepRow(activity, "1", "Open User Settings on the forum and open Two-Factor Authentication."),
                lp(activity, -1, -2, 10, 0));
        body.addView(stepRow(activity, "2", "Open the authentication-app setup so Nearata displays its QR code and Setup key."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "3", "In HCF Authenticator, scan/import the QR code or enter the Setup key shown by Nearata."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "4", "Copy the current 6-digit passcode from HCF Authenticator."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "5", "Return to Nearata setup, enter your forum password and the 6-digit passcode, then press Enable."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "6", "After Nearata confirms activation, save or copy the backup codes it provides."),
                lp(activity, -1, -2, 7, 0));

        Button forumSetup = actionButton(activity, "Open Nearata 2FA Setup", false);
        forumSetup.setOnClickListener(v -> openForumSettings(activity));
        body.addView(forumSetup, lp(activity, -1, dp(activity, 48), 12, 0));

        Button auth = actionButton(activity,
                localReady ? "Open HCF Authenticator" : "Set Up HCF Authenticator",
                true);
        ...
        body.addView(auth, lp(activity, -1, dp(activity, 48), 8, 0));

        TextView statusNotice = text(activity,
                localReady
                        ? "HCF Authenticator is ready locally. Finish or verify activation in Nearata on the forum before treating 2FA as enabled."
                        : "No Nearata setup secret is stored in HCF Authenticator on this device yet.",
                9, color(activity, R.color.hcf_muted, Color.GRAY));
        statusNotice.setGravity(Gravity.CENTER);
        statusNotice.setLineSpacing(0f, 1.08f);
        body.addView(statusNotice, lp(activity, -1, -2, 9, 0));

        TextView privacy = text(activity,
                "Your forum password and Enable action stay in Nearata's forum modal. HCF stores only the TOTP setup secret, encrypted with Android Keystore.",
                9, color(activity, R.color.hcf_hint, Color.GRAY));
        privacy.setGravity(Gravity.CENTER);
        privacy.setLineSpacing(0f, 1.08f);
        body.addView(privacy, lp(activity, -1, -2, 6, 0));

        String summary = localReady
                ? "Authenticator ready locally • verify Nearata status"
                : (security.twoFactorControls
                        ? "Nearata available • QR code or Setup key"
                        : "Open forum to configure Nearata 2FA");
    ...
    private static View infoBox(Activity activity, String title, String message) {
        LinearLayout box = new LinearLayout(activity);
        box.setOrientation(LinearLayout.VERTICAL);
        box.setPadding(dp(activity, 11), dp(activity, 9), dp(activity, 11), dp(activity, 9));
        int cyan = color(activity, R.color.hcf_cyan_bright, Color.rgb(0, 184, 240));
        box.setBackground(roundRect(activity,
                alpha(cyan, 16),
                alpha(cyan, 105),
                11));
        TextView heading = text(activity, title, 11, cyan);
        heading.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        box.addView(heading);
        TextView detail = text(activity, message, 10,
                color(activity, R.color.hcf_muted, Color.LTGRAY));
        detail.setLineSpacing(0f, 1.08f);
        box.addView(detail, lp(activity, -1, -2, 4, 0));
        return box;
    }
    ...
    private static View stepRow(Activity activity, String number, String message) {
        ...
        int cyan = color(activity, R.color.hcf_cyan_bright, Color.rgb(0, 184, 240));
        TextView badge = text(activity, number, 11, cyan);
        badge.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        badge.setGravity(Gravity.CENTER);
        badge.setBackground(roundRect(activity, alpha(cyan, 22), alpha(cyan, 110), 9));
        ...
        copyLp.leftMargin = dp(activity, 10);
    ...
    private static Button actionButton(Activity activity, String label, boolean primary) {
        ...
        if (primary) {
            ...
            button.setBackground(roundRect(activity, cyan, cyan, 12));
        } else {
    ...
    private static TextView twoFactorStatus(Activity activity) {
        ...
        TextView status = text(activity,
                configured ? "Authenticator configured locally" : "Authenticator not configured locally",
                11,
                statusColor);
    ...
    private static void refreshTwoFactor(View root) {
        ...
        if (statusView instanceof TextView) {
            ...
            status.setText(configured
                    ? "Authenticator configured locally"
                    : "Authenticator not configured locally");
            status.setTextColor(statusColor);
            status.setBackground(roundRect(activity,
                    alpha(statusColor, 18), alpha(statusColor, 100), 10));
        }
