...
public final class HcfSubsettingsUi {
    private static final String TAG = "hcf_account_controls_subsettings_ui_v2";
    ...
    private static View buildTwoFactorSubsetting(Activity activity, ForumSecurity.Snapshot security) {
        ...
        body.addView(infoBox(activity,
                "Forum setup uses standard TOTP",
                "The forum shows a QR code and a Setup key. HCF Authenticator can use either one to create the same rotating 6-digit passcode."),
                lp(activity, -1, -2, 10, 0));

        body.addView(stepRow(activity, "1", "Open the forum's Two-Factor Authentication setup page."),
                lp(activity, -1, -2, 10, 0));
        body.addView(stepRow(activity, "2", "In HCF Authenticator, scan/import the QR code or enter the Setup key shown under it."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "3", "Copy the current 6-digit passcode generated by HCF Authenticator."),
                lp(activity, -1, -2, 7, 0));
        body.addView(stepRow(activity, "4", "Return to the forum, enter your forum password and the 6-digit passcode, then press Enable."),
                lp(activity, -1, -2, 7, 0));

        Button forumSetup = actionButton(activity, "Open Forum 2FA Setup", false);
        ...
        Button auth = actionButton(activity,
                isConfigured(activity) ? "Open HCF Authenticator" : "Set Up HCF Authenticator",
                true);
        ...
        TextView setupKeyNote = text(activity,
                "Setup key = the manual key shown by the forum below the QR code. You do not need to know that it is Base32.",
                9, color(activity, R.color.hcf_muted, Color.GRAY));
        setupKeyNote.setGravity(Gravity.CENTER);
        setupKeyNote.setLineSpacing(0f, 1.08f);
        body.addView(setupKeyNote, lp(activity, -1, -2, 9, 0));

        TextView privacy = text(activity,
                "Your forum password stays on the forum page. HCF stores only the authenticator setup secret, encrypted with Android Keystore.",
        ...
        String summary = isConfigured(activity)
                ? "Configured • generates forum 6-digit passcodes"
                : (security.twoFactorControls
                        ? "Available • QR code or Setup key"
                        : "Set up with the forum QR code or Setup key");
