...
public final class SettingsActivity extends ThemedActivity {
    ...
    private View buildHeader() {
        ...
        headerSubtitleView = text(BuildInfo.VERSION_TAG + " • " + channelDisplay(), 10, getColor(R.color.hcf_meta));
    ...
    private void showSettingsHome() {
        ...
        updateSettingsHeader("App Settings", BuildInfo.VERSION_TAG + " • " + channelDisplay());
        ...
        TextView footer = text("Harley's Clan Forum • " + BuildInfo.VERSION_TAG, 9, getColor(R.color.hcf_hint));
    ...
    private void updateSettingsHeader(String title, String subtitle) {
        ...
        if (headerSubtitleView != null) headerSubtitleView.setText(subtitle == null ? BuildInfo.VERSION_TAG + " • " + channelDisplay() : subtitle);
    ...
    private String settingsSectionSubtitle(String key) {
        if ("updates".equals(key) || "about".equals(key)) return BuildInfo.VERSION_TAG + " • " + channelDisplay();
        return "Harley's Clan Forum • " + BuildInfo.VERSION_TAG;
    }
        ...
        updateDownloadButton = actionButton("Download Update Now", v -> {
            if (availableRelease != null && availableRelease.apkUrl != null && !availableRelease.apkUrl.isEmpty()) {
                ...
                if (id > 0L) {
                    ...
                    updateStatus.setText("Downloading Stable build" + (availableRelease.versionCode > 0 ? " " + availableRelease.versionCode : "") + "… The Android installer will open automatically when verification finishes.");
    ...
    private String updateChannelLine(String channel) {
        ...
        return "Channel: " + (stable ? "Stable • Official Releases" : "Dev • Preview Releases");
    }

    private String effectiveUpdateChannel() {
        if (!BuildInfo.ALLOW_UPDATE_CHANNEL_SWITCH) return UpdateChecker.CHANNEL_STABLE;
        ...
        UpdateChecker.check(this, channel, new UpdateChecker.Callback() {
            ...
            public void onResult(UpdateChecker.Release release, boolean updateAvailable) {
                ...
                if (updateAvailable) {
                    ...
                    if (userRequested) Toast.makeText(SettingsActivity.this, "Stable update available" + (release.versionCode > 0 ? " • build " + release.versionCode : ""), Toast.LENGTH_LONG).show();
                } else {
    ...
    private View aboutCard() {
        ...
        TextView appName = text("Harley's Clan Forum", 18, getColor(R.color.hcf_text));
        appName.setTypeface(null, android.graphics.Typeface.BOLD);
        card.addView(appName);

        TextView version = text(BuildInfo.VERSION_TAG + "  •  " + channelDisplay() + "\nBuild " + BuildInfo.VERSION_CODE,
